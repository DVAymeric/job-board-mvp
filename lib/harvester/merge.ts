import type { NormalizedOffer, SourceRef } from "@/lib/harvester/normalized-offer";
...
export function mergeOffers(existing: NormalizedOffer, incoming: NormalizedOffer): NormalizedOffer {
  ...
  // JOB-37 (job-harvester) : base sur `incoming`, pas `existing` — un re-harvest re-normalise
  // systématiquement avec le code le plus récent (mapping de contractType corrigé, titre/
  // description mis à jour côté source, etc.) ; figer sur `existing` gelait silencieusement une
  // offre à l'état de sa toute première collecte. Seule l'identité stable de l'enregistrement
  // (id, provenance primaire, clé de dédup, URL canonique servant à cette clé) reste ancrée sur
  // `existing`.
  return {
    ...incoming,
    id: existing.id,
