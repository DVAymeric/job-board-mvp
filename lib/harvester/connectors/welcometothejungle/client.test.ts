...
describe("fetchWttjOffers", () => {
  it("queries the csekhvms53-dsn.algolia.net jobs index with the expected headers", async () => {
    const fetchImpl = vi.fn<typeof fetch>(async (input, init) => {
      ...
      // Vérifié en direct le 2026-08-20 (job-harvester) : la clé Algolia capturée est restreinte
      // par referer côté Algolia (secured API key) — sans ce header, l'appel échoue en HTTP 403
      // "Method not allowed with this referer", clé pourtant valide.
  ...
  // JOB-audit-2026-08-20 (job-harvester) : joindre tous les mots-clés de la campagne en une
  // seule requête Algolia ("data analyst data quality statistiques BI") donnait 0 résultat en
  // direct, alors que chaque mot-clé pris séparément en donnait des dizaines — Algolia traite la
  // chaîne comme une seule recherche exigeante, pas comme un OU entre mots-clés.
  ...
  // JOB-audit-2026-08-20 (job-harvester) : Algolia matche "BI" par préfixe de mot y compris sur
  // des offres sans rapport ("Biologiste", "Biochimie") — vérifié en direct. Un filtre par
  // limite de mot côté client, comme déjà fait pour Workday, rejette ces faux positifs.
