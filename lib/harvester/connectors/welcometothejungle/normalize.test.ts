...
describe("normalizeWttjOffer", () => {
  ...
  it("never leaks a contact field into rawPayload — the Algolia index doesn't expose one (schema is a strict whitelist)", () => {
    const hits = loadHits();
    const hitWithPii = { ...hits[0]!, contact: { name: "Jean Recruteur", email: "jean@example.com" } };
    const offer = normalizeWttjOffer({ source: "welcometothejungle", payload: hitWithPii });
