...
import { fetchSitemapCrawlerOffers, checkSitemapCrawlerHealth } from "@/lib/harvester/connectors/sitemap-crawler/client";
...
describe("fetchSitemapCrawlerOffers", () => {
  it("only fetches URLs matching a job-relevant path pattern, ignoring the rest", async () => {
    ...
    const results = await collect(fetchSitemapCrawlerOffers(query, { fetchImpl }));
