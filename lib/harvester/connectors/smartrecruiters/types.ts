...
export const SmartRecruitersSearchResponseSchema = z.object({
  ...
  // JOB-32 (job-harvester) : vérifié en direct (curl) — présent sur chaque réponse de liste
  // réelle, utilisé pour paginer plutôt que de supposer que la première page contient tout le
  // catalogue.
...
// JOB-34 (job-harvester) : le slug d'entreprise ciblée (`company`) n'apparaît dans aucun champ
// de la réponse de détail SmartRecruiters elle-même — client.ts l'injecte donc dans ce wrapper
// composite pour que normalize.ts puisse reconstruire une URL de repli réellement valide si
// l'API omet un jour `applyUrl`/`postingUrl`.
