...
describe("normalizeTalentsoftOffer", () => {
  ...
  it("never leaks a field outside link/title/description/categories into rawPayload, even if present on the RSS item object", () => {
    // Les autres champs XML (ex. <pubDate>) ne sont de toute façon jamais extraits par
    // parseRssItems (client.ts) — whitelist par construction (ADR-0004). Vérifié ici au niveau
    // du schéma Zod, deuxième filet si jamais un champ supplémentaire venait à être ajouté à
    // l'objet avant normalize.ts.
    const withExtraField = offerPayload();
    (withExtraField.item as Record<string, unknown>).contactEmail = "jean@example.com";

    const offer = normalizeTalentsoftOffer({ source: "talentsoft", payload: withExtraField });

    expect(offer.rawPayload).not.toHaveProperty("contactEmail");
  });
