...
import { fetchTalentsoftOffers, checkTalentsoftHealth } from "@/lib/harvester/connectors/talentsoft/client";
...
describe("fetchTalentsoftOffers", () => {
  ...
  it("skips a target disallowed by robots.txt without fetching it", async () => {
    // Domaine distinct des autres tests de ce fichier : le résultat de robots.txt est mis en
    // cache par origine au niveau module (lib/harvester/robots.ts) — réutiliser
    // recrutement.mgen.fr ici réutiliserait silencieusement le résultat (permissif) mis en
    // cache par un autre test.
